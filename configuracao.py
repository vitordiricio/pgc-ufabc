...
Este arquivo inclui suporte a:
- Duas faixas por via (mão única), com possibilidade de troca de faixa
...
@dataclass
class Configuracao:
    """Configuração para a simulação com vias de mão única e duas faixas por sentido."""
    ...
    # ============================
    # Configurações da rua/faixas
    # ============================
    # Agora cada via (mão única) tem DUAS faixas de rolamento
    NUM_FAIXAS_POR_SENTIDO: int = 2
    LARGURA_FAIXA: int = 20
    # LARGURA_RUA idealmente = NUM_FAIXAS_POR_SENTIDO * LARGURA_FAIXA
    # Mantemos 40 para compatibilidade com o desenho atual
    LARGURA_RUA: int = 40

    # Divisória entre as duas faixas (visual)
    DESENHAR_DIVISORIA_FAIXA: bool = True
    TAMANHO_TRACO_DIVISORIA: int = 16
    ESPACO_TRACO_DIVISORIA: int = 10
    ...
    # ======== Troca de faixa (lateral) ========
    VEL_LATERAL_TROCA_FAIXA: float = 2.5      # px/frame para drift lateral suave
    JANELA_TROCA_FAIXA: int = 90              # janela longitudinal que deve estar livre
    PROB_TENTAR_TROCA_FAIXA: float = 0.02     # chance de tentativa quando não estritamente necessário

    # ======== Conversão (virar no cruzamento) ========
    PROB_VIRAR_NO_CRUZAMENTO: float = 0.25    # % de veículos que tentam virar ao cruzar
    RAIO_JANELA_CURVA: int = 28               # “miolo” do cruzamento para disparar a curva
    ...
    # Comportamento de faixas e conversões
    PROB_TROCA_FAIXA: float = 0.02  # ~2% por atualização (ajuste à vontade)
    PROB_VIRAR: float = 0.10  # ~10% dos carros decidem virar no cruzamento
