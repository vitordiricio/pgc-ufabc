...
Suporte visual para:
- Informações de duas faixas por via (exibidas no painel lateral)
- Conversões (curvas) habilitadas no cruzamento (exibidas no painel lateral)
"""
import pygame
...
class Renderizador:
    ...
    def renderizar(self, malha: MalhaViaria, info_simulacao: Dict = None) -> None:
        ...
        """
        if info_simulacao is None:
            info_simulacao = {}
        # Desenha o fundo
        self.tela.blit(self.superficie_fundo, (0, 0))

        # Desenha a malha viária (ruas, cruzamentos, veículos)
    ...
    def _desenhar_painel_lateral(self, malha: MalhaViaria, info_simulacao: Dict) -> None:
        ...
        self._desenhar_secao(superficie_painel, "TEMPO", y_texto, [
            ...
            f"FPS (aprox): {info_simulacao.get('fps', 0):.0f}"
        ])
        y_texto += 88
        ...
        # Seção VIAS/FAIXAS (novidades)
        num_faixas = getattr(CONFIG, "NUM_FAIXAS_POR_SENTIDO", 1)
        largura_faixa = getattr(CONFIG, "LARGURA_FAIXA", CONFIG.LARGURA_RUA)
        prob_troca = getattr(CONFIG, "PROB_TROCA_FAIXA", None)
        prob_virar = getattr(CONFIG, "PROB_VIRAR", None)
        raio_curva = getattr(CONFIG, "RAIO_JANELA_CURVA", None)

        itens_vias = [
            f"Faixas por via: {num_faixas}",
            f"Largura da faixa: {largura_faixa}px"
        ]
        if prob_troca is not None:
            itens_vias.append(f"Prob. troca de faixa: {prob_troca:.2f}")
        if prob_virar is not None:
            itens_vias.append(f"Prob. de conversão (virar): {prob_virar:.2f}")
        if raio_curva is not None:
            itens_vias.append(f"Raio da janela de curva: {raio_curva}px")

        self._desenhar_secao(superficie_painel, "VIAS / FAIXAS", y_texto, itens_vias)
    ...
    def _desenhar_controles(self) -> None:
        ...
        col_idx = 0
        linha_h = 18
        col_y = [y_texto, y_texto]

        for tecla, descricao in controles:
            # Tecla
            superficie_tecla = self.fontes['pequena'].render(tecla, True, CONFIG.AMARELO)
            superficie_painel.blit(superficie_tecla, (col_x[col_idx], col_y[col_idx]))
            # Descrição
            superficie_desc = self.fontes['pequena'].render(descricao, True, CONFIG.BRANCO)
            superficie_painel.blit(superficie_desc, (col_x[col_idx] + 60, col_y[col_idx]))
            # Avança posição
            col_y[col_idx] += linha_h
            if col_y[col_idx] > y_texto + 6 * linha_h:
                col_idx = min(1, col_idx + 1)
