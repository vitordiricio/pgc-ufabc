...
class GerenciadorMetricas:
    # (igual ao seu arquivo anterior – sem mudanças)
    # ... conteúdo omitido por brevidade (mantém igual) ...
    def __init__(self):
    ...
    def salvar_relatorio(self, nome_arquivo: str = None) -> str:
        ...
        for h in self.metricas_por_heuristica:
            relatorio['scores'][h.name] = self.calcular_score(h)
...
class Simulacao:
    ...
    def __init__(self, linhas: int = CONFIG.LINHAS_GRADE, colunas: int = CONFIG.COLUNAS_GRADE):
        ...
        # ---------- NOVO: Modo Incidente ----------
        self.modo_incidente: bool = False
    ...
    def processar_eventos(self) -> None:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                ...
                self._processar_clique_mouse(evento)

    def _processar_clique_mouse(self, evento: pygame.event.Event) -> None:
        pos = evento.pos
        # MODO INCIDENTE: cria/remove eventos no segmento clicado
        if self.modo_incidente and CONFIG.INCIDENTES_ATIVOS:
            if evento.button == 1:  # esquerdo: criar bloqueio/lentidão
                mods = pygame.key.get_mods()
                if mods & pygame.KMOD_SHIFT:
                    criado = self.malha.toggle_incidente_por_pos(
                        pos, CONFIG.INCIDENTO_FATOR_LENTIDAO, CONFIG.INCIDENTO_DURACAO_LENTIDAO_S
                    )
                    self._mostrar_mensagem("Lentidão aplicada (20s)" if criado else "Incidente removido")
                else:
                    criado = self.malha.toggle_incidente_por_pos(
                        pos, CONFIG.INCIDENTO_FATOR_BLOQUEIO, CONFIG.INCIDENTO_DURACAO_PADRAO_S
                    )
                    self._mostrar_mensagem("Bloqueio aplicado (15s)" if criado else "Incidente removido")
            elif evento.button == 3:  # direito: remover
                rem = self.malha.remover_incidente_por_pos(pos)
                self._mostrar_mensagem("Incidente removido" if rem else "Nada para remover")
            return  # não processa clique de semáforo nesse modo

        # Clique padrão: controle manual de semáforos (se heurística MANUAL)
        ...
        if resultado:
            ...
            if self.heuristica_atual != TipoHeuristica.MANUAL:
                self._mostrar_mensagem("Ative Manual (5) para clicar semáforo — ou Incidente (I) para eventos")
    ...
    def _processar_tecla(self, evento: pygame.event.Event) -> None:
        if evento.key == pygame.K_ESCAPE:
            ...
            self._mostrar_mensagem(f"Simulação {'Pausada' if self.pausado else 'Executando'}")
        ...
        # ---------- NOVO: alternar Modo Incidente ----------
        elif evento.key == pygame.K_i:
            self.modo_incidente = not self.modo_incidente
            estado = "Ativo" if self.modo_incidente else "Desativado"
            self._mostrar_mensagem(f"Modo Incidente: {estado}")

        elif evento.key == pygame.K_s and (evento.mod & pygame.KMOD_CTRL):
            self._salvar_relatorio()
    
    # ... resto (mudar_heuristica, reiniciar, mensagens, salvar, finalizar, coletar, atualizar, renderizar, executar) 
    # permanece igual ao seu arquivo com a única diferença de _processar_clique_mouse/_tecla acima ...
    def _mudar_heuristica(self, nova_heuristica: TipoHeuristica) -> None:
        if nova_heuristica != self.heuristica_atual:
            ...
            self._mostrar_mensagem(f"Heurística: {nomes.get(nova_heuristica, 'Desconhecida')}")
